...
void handleRoot() {
  ...
  */
  ...
  html += "<div class=\"row\"><label for=\"cathode_start\">What hour to start doing 10 minute cathode healing cycles (even when sleeping) (0-23) -1 disables cycles:</label>";
  ...
  html += FW_VERSION;
  html += ", check out <a href=\"https://github.com/mcer12/Nick-ESP8266\" target=\"_blank\"><strong>NICK</strong> on GitHub</a>";
  #ifdef T_DEBUG
  html += " | Generated at ";
  char nstr[4];
  sprintf(nstr, "%02d", hour());
  html += nstr;
  html += ":";
  sprintf(nstr, "%02d", minute());
  html += nstr;
  html += " | Cathode Value: ";
  html += json["cathode"].as<const char*>();
  html += " | Whistle ONE: ";
  sprintf(nstr, "%s", ((json["cathode"].as<int>() == 2 && (inHealRange && currentHour % 2 == 0 && minute() <= 10) || (minute() > 1 && currentHour % 2 == 0))) ? "1" : "0");
  html += nstr;
  html += " | Hour() % 2: ";
  sprintf(nstr, "%d", hour()%2);
  html += nstr;
  #endif
  html += "</p></div>";
